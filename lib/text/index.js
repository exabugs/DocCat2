...
var util = {
  ...
  /*
   innerproduct: function (array, keyword) {
   var sum = 0;
   for (var i = 0, len = array.length; i < len; i++) {
   var term = array[i];
   if (keyword[term.k]) {
   sum += term.c * keyword[term.k];
   }
   }
   return sum;
   },
...
exports.search = function (collection, condition, text, callback) {
  ...
  tasks.push(function (done) {
    exports.parse(text, function (err, keyword) {
      done(err, keyword);
    });
  });
  tasks.push(function (keyword, done) {
    ...
    var map = function () {
      emit(this._id, util.innerproduct(this[attr].tf, keyword));
    };
