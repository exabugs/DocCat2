...
exports.to_tfiof = function (tf_array, field, of_coll, option, callback) {
 ...
 * @param callback
 */
exports.countup = function (master_collection, master_option, match, collection, attribute, field, option, callback) {

  var condition = master_option.condition || {};
  var cursor = master_collection.find(condition);
  ...
  cursor.count(function (err, count) {
    ...
    cursor.each(function (err, item) {
      if (item) {
        ...
        option.condition[match] = {$in: [item._id]};
        exports.term_frequency(collection, attribute, field, option, function (err, results) {
          var array = [];
          ...
          var master_attribute = master_option.attribute || attribute;
          var update = {};
          update[master_attribute] = array;
          master_collection.findAndModify({_id: item._id}, [], {$set: update}, function (err) {
            if (--c === 0) {
              return callback(err, count);
            }
          });
