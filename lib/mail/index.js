...
//  , mongodb = require('mongodb')
  , mongodb = require('node-searcher').mongodb
...
//  , searcher = require('node-searcher')
  , searcher = require('node-searcher').text
  ;
  ...
  async.waterfall([
    ...
    function (db, mailInfo, done) {
      var condition = {
        ...
        'metadata.origin._id': mailInfo.origin._id
      };
      searcher.batch(db, COLLECTION, COLLECTION + "_info", condition, ['k', 'c'], function (err, count) {
        done(err, db, mailInfo);
      });
    }
...
function _insert(db, mailInfo, done) {
  var metadata = {origin: {_id: mailInfo.origin._id, messageId: mailInfo.messageId}};
  async.waterfall([
    function (done) {
