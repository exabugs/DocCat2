...
function add(path, callback) {
  ...
  async.waterfall([
    ...
    function (db, done) {
      ...
      var gs = new mongodb.GridStore(db, _id, 'w', option);
      gs.writeFile(path, function (err) {
        done(err, db, _id);
      });
    },
    // GridStore取得
    function (db, _id, done) {
      var option = {root: GRID_ROOT, content_type: 'message/rfc822'};
      var gs = new mongodb.GridStore(db, _id, 'r', option);
...
function _insert_files(db, array, callback) {
  async.each(array, function (file, next) {
    var option = {content_type: file.contentType, root: GRID_ROOT};
